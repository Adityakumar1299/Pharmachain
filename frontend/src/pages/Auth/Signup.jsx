...
// import "../styles/Signup.css"; // Assuming you have this file
...
export default function Signup() {
  ...
  const handleChange = (e) => {
    ...
    // Handle 'yearsOfExperience' as a number
  ...
  const validateStep = () => {
    ...
    // Specific check for yearsOfExperience
    if (step === 2 && (formData.role === 'doctor' || formData.role === 'pharmacist')) {
        if (formData.yearsOfExperience < 0) {
            currentErrors.yearsOfExperience = "Years must be a positive number.";
        }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateStep()) {
      ...
      const { role, confirmPassword, ...dataToSend } = formData;
      const API_URL = `http://localhost:8000/auth/signup/${role}`;
      ...
      try {
        ...
        setMessage(`Registration for ${role} successful! You can now log in.`);
        // Reset form to initial state for that role
        if (role === "doctor") setFormData(initialDoctorState);
        else if (role === "pharmacist") setFormData(initialPharmacistState);
        else if (role === "patient") setFormData(initialPatientState);
        setStep(1);
      ...
      <div className={`form-group ${options.span2 ? "grid-col-span-2" : ""}`}>
        ...
        <div className="input-wrapper">
          <input
            ...
            onChange={handleChange}
            placeholder={options.placeholder || ""}
            min={type === "number" ? "0" : undefined} // Ensure numbers are not negative
          />
          {isPassword && (
            <span
  ...
  const renderStep1 = () => (
    <div className="form-grid">
      ...
      {renderField("fullName", "Full Name", "text", {
        placeholder: "e.g., John Doe",
      })}
      {renderField("email", "Email Address", "email", {
        placeholder: "you@example.com",
      })}
      {renderField("password", "Create Password", "password", {
  ...
  const professionalSteps = {
    doctor: () => (
      <div className="form-grid">
        {renderField("licenseNumber", "Medical Council License Number", "text", {
          span2: true,
        })}
        {renderField("speciality", "Speciality")}
        {renderField("yearsOfExperience", "Years of Experience", "number")}
        {renderField("qualification", "Highest Qualification", "text", {
          span2: true,
        })}
      </div>
    ),
    pharmacist: () => (
      <div className="form-grid">
        {renderField("licenseNumber", "Pharmacy Council License Number", "text", {
          span2: true,
        })}
        {renderField("qualification", "Highest Qualification")}
        {renderField("yearsOfExperience", "Years of Experience", "number")}
    ...
    patient: () => (
      <div className="form-grid">
        ...
        {renderField("pincode", "Pincode")}
        {renderField("aadharNumber", "Aadhar Number (for verification)")}
        {renderTextarea("allergies", "Known Allergies or Conditions", {
          span2: true,
          placeholder: "e.g., Penicillin, Pollen, Asthma",
        })}
      </div>
    ),
  };
  ...
  const contactSteps = {
    doctor: () => (
      <div className="form-grid">
        {renderField("hospitalName", "Hospital / Clinic Name", "text", {
          span2: true,
        })}
        {renderField("hospitalAddress", "Hospital / Clinic Address", "text", {
          span2: true,
        })}
        {renderField("city", "City")}
        {renderSelect("state", "State", indianStates)}
    ...
    pharmacist: () => (
      <div className="form-grid">
        {renderField("pharmacyName", "Pharmacy Name", "text", { span2: true })}
        {renderField("addressLine1", "Pharmacy Address", "text", {
          span2: true,
        })}
        {renderField("city", "City")}
        {renderSelect("state", "State", indianStates)}
    ...
    <div className="signup-container">
      <div className="signup-wrapper">
        ...
        <div className="signup-form-panel">
          ...
          {message && (
            <p className={`feedback-msg ${message.includes("Error") ? "error" : "success"}`}>
                {message}
            </p>
          )}
        </div>
